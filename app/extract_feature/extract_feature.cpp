...
class Classifier {
 public:
  ...
  const boost::shared_ptr<Blob<float> > getFeature(string layerName);
...
const boost::shared_ptr<Blob<float> > Classifier::getFeature(std::string layerName){
    return net_->blob_by_name(layerName);
}
...
int main(int argc, char** argv) {
  ...
  while(fin>>file){
      ...
      for (int i = 0; i < num_features; ++i) {
          if(beginFlag)
              fout.open((featureNames[i]+".out").c_str(),std::fstream::app);
          else
              fout.open((featureNames[i]+".out").c_str()),beginFlag=true;
          const boost::shared_ptr<Blob<float> > feature_blob = classifier.getFeature(featureNames[i]);
          ...
          for (int n = 0; n < batch_size; ++n) {
              ...
              if (image_indices[i] % 1000 == 0) {
                  ...
                  " query images for feature blob " << featureNames[i];
              }
          }  // for (int n = 0; n < batch_size; ++n)
          fout.close();
      }  // for (int i = 0; i < num_features; ++i)
