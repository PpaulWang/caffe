...
class featureExtractor {
 public:
  featureExtractor(const std::string& model_file,
             const std::string& trained_file,
             const std::string& mean_file);

  void Forward(const cv::Mat& img);
  const boost::shared_ptr<std::vector<float> > getFeature(std::string layerName);
...
featureExtractor::featureExtractor(const std::string& model_file,
                       const std::string& trained_file,
                       const std::string& mean_file) {
#ifdef CPU_ONLY
...
const boost::shared_ptr<std::vector<float> > featureExtractor::getFeature(std::string layerName){
...
void featureExtractor::SetMean(const std::string& mean_file) {
...
void featureExtractor::Forward(const cv::Mat& img) {
  Blob<float>* input_layer = net_->input_blobs()[0];
